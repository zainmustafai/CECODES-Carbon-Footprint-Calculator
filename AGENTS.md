...
(resolve it from this file's directory; in a monorepo the `next` package may not be visible from
the repo root). Heed deprecation notices.

Next used to inject this section itself on every `next dev`, marker comments and all. Its 16.3
wording carries an em dash, which this repo bans, so one dev run was enough to turn the suite red.
`agentRules: false` in next.config.ts turns that generator off; the guidance is kept here by hand.
