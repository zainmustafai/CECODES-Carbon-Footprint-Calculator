...
export function SourceRow({ source }: { source: SourceVM }) {
  const t = useTranslations("dataEntry.source");
  const cell = source.cells[0];
  if (!cell) return null;

  return (
    <div className="flex flex-col gap-3 border-t py-3 first:border-t-0 md:grid md:grid-cols-[1fr_14rem_auto] md:items-center">
