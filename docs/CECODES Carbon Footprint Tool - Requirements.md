...
> **Status:** CECODES answered the round-1 decision memo on **2026-07-17**, round 2 on **2026-07-18**, and the final numbered questions on **2026-07-24**, attaching the **DASHBOARD workbook** (`docs/sample-data/CEC-PR-F-024 - DASHBOARD (2025).xlsx`, "the important pages are PRINCIPAL, Emission Factors and DASHBOARD"). Items marked **DECIDED** below are settled and built to; their answer is quoted. Round 2 is [CLIENT_DECISION_MEMO_ROUND2.md](./CLIENT_DECISION_MEMO_ROUND2.md).

**A. Source-data / methodology decisions**

1. **DECIDED (2026-07-17), superseded in detail (2026-07-24).** **The authoritative factor table** is now the **"Emission Factors" sheet of the DASHBOARD workbook** (same per-gas column layout as the old "Jerarquia nueva (2025)" sheet, newer values: UPME 2025 present, USD decimals corrected). The 2026-07-25 sync reconciled the library to it, preserving every admin-edited factor. The old Scope-2 caveat is closed by answer 4: the DASHBOARD sheet governs electricity (see A9).
   **NEW QUESTION for CECODES (2026-07-25): the urea factor looks misplaced.** _"Fertilización con Urea (CO(NH2)2)"_ carries **0.7333 labelled "kg CO2e/kg urea"** (= 0.2 × 44/12, the IPCC's **CO₂** emission from urea) **in the N₂O column**, so the workbook, and therefore the tool, multiplies it by the N₂O GWP of 273: 32,626 kg of urea computes as **6,531.7 t CO₂e instead of ≈23.9 t**. Parity reproduces it faithfully; whether it is intended needs their answer.
...
3. **DECIDED (2026-07-24) and APPLIED.** **Purchased-goods spend factors** are **per USD**, and the "implausibly high" values were a **decimal-comma misreading**, not a unit error. CECODES: _"I'm almost sure it's reported by usd. Value is not good expressed, 2,893 means two point eight nine three, not two thousand"_ (same for _"Ganadería y Agricultura de Doble Propósito"_ = 2.893 and _"Ganado lechero y producción de leche"_ = 1.724, not 1,700). The DASHBOARD workbook's Emission Factors sheet carries the correctly-expressed values (cement 3.924 kg CO₂e/USD, etc.) and the 2026-07-25 library sync imported them. **A4 is unblocked.**
4. **DECIDED (2026-07-18), scope clarified by A3's resolution.** **Currency for purchases** CECODES: _"COP/USD let's use the year average rate."_ With A3 resolved (factors are genuinely **per USD**), the tool matches the Excel exactly as-is: the spend elements' consumption unit **is USD**, the company enters its spend in USD, and the engine prices it directly, no conversion step in the calculation. The year-average TRM matters only as a **convenience for companies whose books are in COP** (converting before entry, exactly as they would for the Excel). A TRM helper table remains a possible later convenience, not a parity requirement, because the reference spreadsheet itself takes USD input.
5. **RESOLVED (2026-07-24), empirically, and APPLIED.** **GWP edge cases** the DASHBOARD workbook's calculation formulas multiply by the **AR6 column unconditionally**, whatever period is entered, and §14.1 makes that spreadsheet the acceptance test. `resolveGwpSet` therefore returns **AR6 for every year**; years created earlier keep their pinned `gwpSet`, so nothing already entered is restated. The AR5 constants were aligned to the workbook's own GWP sheet (CH₄ fossil 30 / non-fossil 28) for those pinned rows. **`MIN_REPORTING_YEAR` stays 2000 and the 2025-floor debate is closed the other way:** with the full UPME grid series 2008-2025 now loaded (see A9), every recent year computes, and CECODES's own 2026-07-24 examples are **2024** data, so forbidding pre-2025 years would forbid exactly what they demonstrated doing. **Biogenic CO₂** _(biogénica)_ remains **included in the headline total and disclosed separately** (CECODES: _"I like your suggestion so please include it"_).
...
**A9. RESOLVED (2026-07-24) and APPLIED.** **The 2025 grid electricity factor exists:** the DASHBOARD workbook's Emission Factors sheet carries _"Energía eléctrica adquirida (Factor emisión UPME 2025)"_ = **0.097018445 kg CO₂e/kWh** (CECODES quoted it as _"0,097"_), and answer 4 names that sheet the reference for the years where the two old sheets disagreed: _"2025, or please use dashboard excel table as reference."_ The 2026-07-25 sync (`import-factors.ts --apply-grid`) loaded the **full UPME series 2008-2025**: 12 missing years created (including 2025) and 5 mismatched years corrected to the sheet (2024 → 0.21742, matching the workbook's own sample). The zero-valued RECs row remains outside `grid_electricity_factors` (see item 13).
...
13. **DECIDED (2026-07-18), BUILT (2026-09-04).** **Self-generated renewable energy** (Scope 2) CECODES: _"Renewable energy will be reported separately."_ Track separately and disclose, not folded into the Scope 2 grid total. Self-generation is reported in the free-form "tecnologías más limpias" table (B15), which carries no factor and affects no total. The related RECs row (_"...respaldada con RECs (cualquier año)"_ = 0) is now modelled: it is its own Alcance 2 element worth 0 kg CO2e/kWh, seeded by `ensureRecElectricityFactor` in `prisma/seed.ts` because the importer keys Alcance 2 rows by the year in the element name and this row has none. The engine reads an entry's own per-kWh factor in preference to the year's grid factor (`scope2RatePerKwh` in `src/lib/calc/rollup.ts`), so REC-backed consumption prices at zero instead of being charged full grid emissions, and stays priceable in a year with no grid factor loaded.

**B15. NEW REQUIREMENT (2026-07-24), BUILT (2026-07-25).** **"Datos sobre tecnologías más limpias y buenas prácticas."** CECODES: _"la sección ... debería quedar abierta para que los que diligencien el formulario puedan reportar esta información de una manera más libre"_; reported values _"will not affect the calculations, they'll only appear at summary table."_ Built as a free-form table on the data-entry screen (columns per their screenshot: Alcance, Categoría, Subcategoría, Elemento, Dato de actividad, Unidad Consumo; only the element is required), shown verbatim on the Resumen, in the Excel export (own sheet) and the PDF (own section), with **no totals and no effect on any calculation**. Note the DASHBOARD workbook's own BASE_evitadas table DOES compute "avoided emissions" from these rows; the client's instruction explicitly chose the open, non-calculating version, so that computation is deliberately not reproduced.
...
> **Item 0 (the blocking one) is RESOLVED (2026-07-24) and the acceptance test PASSES.** CECODES sent the DASHBOARD workbook, whose **PRINCIPAL** sheet contains a filled-in sample company (four sedes; emissions, removals, monthly-energy and "tecnologías más limpias" tables) together with the Excel's **own cached formula results** per row and in total. That is exactly the "activity data as entered AND the totals produced from it" this item demanded. The client-origin parity fixture (`src/lib/calc/__tests__/fixtures/parity/cecodes-dashboard-principal-2024.json`) transcribes it, and `rollupYear` reproduces every total: emissions 11,852.83 t across the three scopes plus 16.12 t of monthly energy, and removals **separately** at -256.40 t. Two questions this settled empirically: the **CH₄ GWP selector is the biogenic flag** (the formulas execute `IF(flag=1 → ×27, flag=0 → ×29.8)`; the round-2 "mark the fuels" list is moot) and the **GWP set is AR6 for every year** (see A5). One anomaly is reproduced faithfully and flagged back to CECODES: the urea factor (see item 1). The separately-shared Power BI screenshots (Cultivos Casablanca, 2024, 2.05k t) remain useful as a second, screenshot-level cross-check only.
