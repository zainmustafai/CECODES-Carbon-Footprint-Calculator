...
// globalSetup runs in Playwright's own Node process, which does not read .env.local. It needs
// DIRECT_URL (falling back to DATABASE_URL) and nothing else: it opens its own pg client and
// writes the fixture tenant, including the accounts, straight into the schema. There is no
// provider to hold an API key for any more.
loadEnvConfig(process.cwd());

const BASE_URL = process.env.E2E_BASE_URL ?? "http://localhost:3000";

// One worker. The suite runs against whatever database DIRECT_URL names, inside a disposable,
// uniquely named company that global setup creates and teardown removes; a second worker would be
// a second run fighting the first over the same fixture rows. docker-compose.yml ships a local
// Postgres, so pointing this at a throwaway database is now the easy option rather than the
// awkward one. Never point it at a database whose data you care about.
export default defineConfig({
  ...
  // hit and every query is a real database round trip. The data-entry happy path in particular
  ...
  // on first hit and hits the database on every query. Raising it to 10s removes the borderline
  // flakes on filtered lists and post-mutation re-renders without weakening any check:
